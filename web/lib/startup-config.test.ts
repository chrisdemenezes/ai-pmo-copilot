...
// kill switch (DISABLE_WORKSPACE_SESSION_GATE, web/proxy.ts) must fail the
// boot outside dev. Mandate letters M-R.
describe("DISABLE_WORKSPACE_SESSION_GATE fail-fast", () => {
  it("M: DEV + gate normal (unset) has no problems", () => {
    vi.stubEnv("DISABLE_WORKSPACE_SESSION_GATE", undefined);
    expect(collectStartupConfigProblems("dev")).toEqual([]);
    expect(() => validateStartupConfig("dev")).not.toThrow();
  });

  it("N: DEV + kill switch true is preserved -- no problem, boot succeeds", () => {
  ...
  describe.each(["staging", "production"] as const)("%s", (environment) => {
    ...
    it(`O/Q: ${environment} + gate normal (unset) has no problems`, () => {
      setValidEnv();
      vi.stubEnv("DISABLE_WORKSPACE_SESSION_GATE", undefined);
    ...
    it(`O/Q: ${environment} + gate normal (explicit "false") has no problems`, () => {
      setValidEnv();
      vi.stubEnv("DISABLE_WORKSPACE_SESSION_GATE", "false");
      expect(collectStartupConfigProblems(environment)).toEqual([]);
      expect(() => validateStartupConfig(environment)).not.toThrow();
    });

    it(`P/R: ${environment} + kill switch true fails the boot`, () => {
      setValidEnv();
      vi.stubEnv("DISABLE_WORKSPACE_SESSION_GATE", "true");
      expect(collectStartupConfigProblems(environment)).toEqual(
        expect.arrayContaining([expect.stringContaining("DISABLE_WORKSPACE_SESSION_GATE")]),
      );
      expect(() => validateStartupConfig(environment)).toThrow(StartupConfigError);
    });

    it(`${environment} + kill switch true is reported alongside other missing config, never masked`, () => {
      vi.stubEnv("SESSION_SECRET", undefined);
      vi.stubEnv("DISABLE_WORKSPACE_SESSION_GATE", "true");
      ...
      expect(problems).toEqual(
        expect.arrayContaining([
          ...
          expect.stringContaining("DISABLE_WORKSPACE_SESSION_GATE"),
        ]),
      );
    });
  });
