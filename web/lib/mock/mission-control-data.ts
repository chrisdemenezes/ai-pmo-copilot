...
export const ENTERPRISE_PROGRAM_WAVES: WaveEntry[] = [
  ...
  },
  {
    ...
    detail: "Corresponde às Releases 0.4/0.5 já aprovadas (Integration Hub, Event Orchestration). Founder autorizou o início da Wave 4 (D-070); nomenclatura oficial do roadmap harmonizada (D-071) e aprovada/concluída (D-072). Founder Kickoff formal (D-073): Domain Blueprint produzido. Decision Proposal resolvida (D-074): `pmo_workflow.py` classificado como Historical Superseded Architecture. AR-7 (D-075): Architecture Review concluída, veredito GO. AR-7 aprovada pelo Founder (D-076): Technical Design produzido -- as 4 condições obrigatórias resolvidas. Epic W4-1 implementado (D-077): migração atômica de `EventEmitter`/`NoOpEventEmitter` (Wave 1) para `EventPublisher`/`InProcessEventPublisher` + `EventDispatcher`, `DomainEvent` (envelope único), migração 0018 (`events`/`dead_letter_events`), Retry/Dead Letter mínimo (`MAX_ATTEMPTS=3`), `correlation_id` reusando `RequestIDMiddleware`/`request_id_var` já existente na plataforma. Os 5 produtores existentes migrados sem alteração funcional; suíte completa verde (503 testes). GO para o Epic W4-2 -- **ciclo institucional aguarda Executive Review antes da autorização do próximo Epic**, per determinação explícita do Founder. Enterprise Analytics (D-072) é uma capacidade transversal construída ao longo desta Wave e das Waves 5/6, não uma Wave própria.",
  },
  {
    code: "Wave 5",
