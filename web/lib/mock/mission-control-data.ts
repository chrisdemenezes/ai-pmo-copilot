...
export const ENTERPRISE_PROGRAM_WAVES: WaveEntry[] = [
  ...
  },
  {
    ...
    detail: "Corresponde às Releases 0.4/0.5 já aprovadas (Integration Hub, Event Orchestration). Founder autorizou o início da Wave 4 (D-070); nomenclatura oficial do roadmap harmonizada (D-071) e aprovada/concluída (D-072). Founder Kickoff formal (D-073): Domain Blueprint produzido -- levantamento obrigatório (5 eventos reais hoje, todos em DomainService, sem envelope de observabilidade completo; 2 workflows manuais síncronos sem evento; zero precedente de fila/retry/dead-letter; achado crítico em `src/workflows/pmo_workflow.py`) + modelo operacional (Event Model/Contracts/Publisher/Dispatcher, Workflow Runtime mínimo, Execution Tracking, Retry/Dead Letter, Integration Gateway, Event Audit como extensão da auditoria de domínio). Decision Proposal resolvida (D-074): `pmo_workflow.py` classificado como Historical Superseded Architecture, não removido, não representa a arquitetura vigente, nenhuma reutilização para a Wave 4. Autorizado a seguir para Architecture Review antes de qualquer Technical Design ou implementação. Enterprise Analytics (D-072) é uma capacidade transversal construída ao longo desta Wave e das Waves 5/6, não uma Wave própria.",
  },
  {
    code: "Wave 5",
