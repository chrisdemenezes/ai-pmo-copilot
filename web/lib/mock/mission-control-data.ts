...
export const ENTERPRISE_PROGRAM_WAVES: WaveEntry[] = [
  ...
  },
  {
    ...
    detail: "Corresponde às Releases 0.4/0.5 já aprovadas (Integration Hub, Event Orchestration). Founder autorizou o início da Wave 4 (D-070); nomenclatura oficial do roadmap harmonizada (D-071) e aprovada/concluída (D-072). Founder Kickoff formal (D-073): Domain Blueprint produzido. Decision Proposal resolvida (D-074): `pmo_workflow.py` classificado como Historical Superseded Architecture. AR-7 (D-075): Architecture Review concluída, veredito GO. AR-7 aprovada pelo Founder (D-076): Technical Design produzido. Epic W4-1 implementado (D-077): migração atômica de `EventEmitter`/`NoOpEventEmitter` para `EventPublisher`/`InProcessEventPublisher` + `EventDispatcher`, `DomainEvent` (envelope único), migração 0018 (`events`/`dead_letter_events`), Retry/Dead Letter mínimo, `correlation_id` reusando `RequestIDMiddleware`/`request_id_var` já existente na plataforma. Executive Review do Epic W4-1 aprovada, GO (D-078): Founder confirmou o pacote de evidências e encerrou formalmente o Epic W4-1; harmonização documental do Technical Design registrada (sem nova decisão arquitetural). Ciclo institucional do Epic W4-2 aberto -- achado de reconciliação de escopo apresentado (Event Dispatcher/Event Audit/Retry-Dead Letter, originalmente atribuídos a W4-2/W4-5, já entregues no W4-1; escopo remanescente de W4-2 é apenas Event Metrics), **aguardando decisão explícita do Founder sobre o escopo antes do Technical Design de W4-2**. Enterprise Analytics (D-072) é uma capacidade transversal construída ao longo desta Wave e das Waves 5/6, não uma Wave própria.",
  },
  {
    code: "Wave 5",
