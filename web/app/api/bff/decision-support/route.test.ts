...
const SAMPLE = {
  ...
  citations: [{ advisor_names: ["risk_advisor"], source_type: "analysis_record", source_id: 7, source_label: "Risk 7" }],
  composition_trace: {
    selection_signals: ["risco", "entrega"],
...
describe("POST /api/bff/decision-support", () => {
  beforeEach(() => {
  ...
  it("returns 503 without leaking detail when BACKEND_URL or API_KEY are unset", async () => {
    ...
    const response = await POST(
      requestWith({ question: "Existe algum risco relevante?", scope: { type: "organization" } }),
    );
  ...
  it("returns 401 when there is no session cookie", async () => {
    const response = await POST(
      requestWithoutSession({ question: "Existe algum risco relevante?", scope: { type: "organization" } }),
    );
  ...
  it("rejects a question shorter than 3 characters before contacting the backend", async () => {
    ...
    const response = await POST(requestWith({ question: "Oi", scope: { type: "organization" } }));
  ...
  it("rejects a request with no scope before contacting the backend (Explicit Scope, Vision Princípio 13)", async () => {
    ...
    const response = await POST(requestWith({ question: "Existe algum risco relevante?" }));
  ...
  it("forwards question and scope verbatim to the backend", async () => {
    const fetchMock = vi
      .fn()
    ...
    expect(String(calledUrl)).toBe("http://backend.test/api/decision-support/ask");
  ...
  it("sends institutional headers resolved from the session cookie", async () => {
    ...
    await POST(
      requestWith(
        { question: "Existe algum risco relevante?", scope: { type: "organization" } },
  ...
  it("maps a 404 from the backend to a scope_not_found error (cross-tenant/nonexistent Project or Portfolio)", async () => {
    ...
    const response = await POST(
      requestWith({ question: "Como está o portfólio?", scope: { type: "portfolio", portfolio_id: 999 } }),
    );
  ...
  it("maps a 422 from the backend to invalid_request", async () => {
    ...
    const response = await POST(
      requestWith({ question: "Existe algum risco relevante?", scope: { type: "project", project_id: 1 } }),
    );
    expect(response.status).toBe(400);
    expect((await response.json()).error).toBe("invalid_request");
  });
