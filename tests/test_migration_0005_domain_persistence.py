...
def test_0005_creates_portfolios_and_programs_tables():
    with temp_database_url("migration_0005") as database_url:
        env = os.environ.copy()
...
def test_0005_extends_projects_table_instead_of_creating_projects_delivery():
    with temp_database_url("migration_0005_project") as database_url:
        env = os.environ.copy()
        env["DATABASE_URL"] = database_url
...
def test_0005_new_project_columns_are_nullable_and_dont_break_legacy_rows():
    with temp_database_url("migration_0005_legacy") as database_url:
        env = os.environ.copy()
        env["DATABASE_URL"] = database_url
...
def test_0005_downgrade_is_lossless_and_full_round_trip_works():
    with temp_database_url("migration_0005_downgrade") as database_url:
        env = os.environ.copy()
        env["DATABASE_URL"] = database_url
