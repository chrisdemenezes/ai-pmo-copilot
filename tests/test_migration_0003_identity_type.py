...
def test_0003_adds_identity_type_with_backfilled_default(tmp_path):
...
def test_0003_downgrade_removes_the_column_and_is_lossless_otherwise(tmp_path):
    db_path = tmp_path / "migration_0003_downgrade.db"
    database_url = f"sqlite:///{db_path}"
    env = os.environ.copy()
    env["DATABASE_URL"] = database_url
