...
| 8 | TD-008 Fase 3b (Wave 2/3) | ✅ **RESOLVIDO — todas as etapas concluídas (1, 2, 3, 5, 4a e 4b)** — D-056/D-057/D-058/D-059/D-060/D-061. **Etapa 4b (destrutiva, concluída):** migração `0015` ativada (`alembic heads` = `0015`) — `NOT NULL` em `analysis_records.project_id` + `DROP COLUMN analysis_records.project_name` + drop do índice; campo `project_name` removido do ORM. `project_id` é a única chave de acesso interno ao Project. **Nome preservado como apresentação** (Condição 1 do Founder): `Project.name` é a fonte do nome de exibição; o campo `project_name` permanece nas responses de intelligence, derivado exclusivamente de `Project.name` — zero regressão de frontend. **Downgrade íntegro** (Condição 2): recria a coluna + índice, relaxa `project_id` para nullable e repopula `project_name` de `projects.name` via `project_id`; provado em PostgreSQL real (`tests/test_migration_0015_drop_project_name.py`: upgrade dropa a coluna, downgrade restaura com o nome real do Project). **Validação** (Condição 3): ruff limpo; pytest **450 passando**; tsc/eslint limpos; vitest **491**; E2E **292 passando (2 skipped)** — os 2 skips são o mesmo teste único `e2e/shell.spec.ts:92` (verificação mobile-only da bottom nav) que se auto-pula nos projetos `lg`/`md` e roda no `mobile`, pré-existente e sem relação com TD-008; zero skips novos em qualquer suíte. Busca global confirma zero dependências comportamentais de `project_name` (coluna ausente do schema e do ORM). Arquivos removidos: nenhum arquivo inteiro — o campo `project_name` foi removido da classe `AnalysisRecord` (`src/database/repository.py`) e da tabela `analysis_records`. **Encerramento** (Condição 4): todos os critérios satisfeitos. **TD-008 declarado RESOLVIDO.** **Gate Final de Migração aprovado** (veredito NÃO-GO para a destrutiva estava correto — a governança funcionou; `docs/product/governance/TD-008-PHASE-3B-GATE-FINAL-DE-MIGRACAO.md`). **Etapa 4a (aditiva, reversível — eliminação dos consumidores residuais R1-R6):** `project_id` é a única chave de escopo de leitura — `list_analyses` id-only (`resolve_scope_id` reutilizado pelo serviço e pelo `AIContextEngine`); display derivado de `Project.name` via `AnalysisRecord.project`/`analysis_display_name`; `list_latest_risks` dedup por id, `summarize_portfolio` agrupa por id; responses e joins de frontend (`decision-queue`/`portfolio-view`) por `project_id`; `save_analysis` não grava mais a coluna. `ProjectSummaryResponse`/`ProjectSummaryService` mantidos e classificados como projeção de leitura/serviço de composição (`DOMAIN-MODEL.md`). Migração destrutiva `0015` **definida e encenada** (`alembic/versions_pending/`, fora do head ativo `0014`) com upgrade/downgrade **provados reversíveis em PostgreSQL real** (`tests/test_migration_0015_drop_project_name.py`). ruff limpo, pytest 449 + teste 0015, tsc/eslint limpos, vitest 491, E2E 292. **Novo Gate + nova aprovação explícita do Founder** são exigidos antes da Etapa 4b (ativar a 0015: `NOT NULL` + `DROP COLUMN` + remoção dos contratos temporários). **Etapa 5 (eliminação de `ProjectSummary`, reordenada pelo Founder para antes da Etapa 4):** `ProjectSummary` + `WorkspaceSummary` (dois espelhos duplicados do read-model de inteligência de um Project) consolidados no tipo canônico `ProjectIntelligenceSummary`, ancorado em `project_id`; todos os consumidores migrados, definições antigas removidas; rejeitada a fusão com a entidade de Entrega (bounded context distinto). Backend inalterado. tsc/eslint limpos, vitest 486, E2E 292. **Falta a Etapa 4** (destrutiva), condicionada ao **Gate Final de Migração** + nova aprovação explícita. **Etapas 1, 2 e 3 de 5 (aditivas)** — D-056/D-057/D-058. **Etapa 3 (frontend-only, consumidores usam project_id como chave primária):** hook compartilhado `useResolvedProjectId` reaproveita a resolução nome→id do summary (deduplicada pelo React Query, sem resolução redundante); os consumidores escopados restantes (`RisksPanel`, `CommunicationBrief`, `IntelligenceTimeline`, `AnalysisHistory`, `ActionsSection`, `ActionsContextLine`) enviam `project_id` como chave exata; `keepPreviousData` evita flash na troca de chave; independência de painéis preservada e reflexo pós-mutação inalterado (invalidações por prefixo). `project_name`/`ProjectSummary` não removidos (Etapa 4/5). tsc/eslint limpos, vitest 486, E2E 292. Backend inalterado. Próxima: Etapa 4 (**bloqueada** — DROP COLUMN exige nova aprovação). **Etapas 1 e 2 de 5 (aditivas)** — D-056/D-057. **Etapa 2 (frontend-only, dual-key nos consumidores):** as 4 rotas BFF de intelligence encaminham `project_id` opcional; `WorkspaceSummary` + os hooks escopados carregam `project_id` coexistindo com o nome; `executive-brief.tsx` reaproveita o `project_id` resolvido pelo summary como chave exata nas leituras irmãs (independência de painéis preservada). tsc/eslint limpos, vitest 483, E2E 292. Backend inalterado. Próxima: Etapa 3 (consumidores usam `project_id` como chave primária). **Etapa 1 de 5 (aditiva, backend)** — D-056. Tratado como migração arquitetural controlada e auditável: Impact Assessment (`docs/architecture/TD-008-PHASE-3B-IMPACT-ASSESSMENT.md`, Fases 1+2) produzido e aprovado pelo Founder antes de qualquer código; execução em 5 etapas, cada uma concluída/testada/documentada/commitada separadamente. **Etapa 1 (aditiva, zero remoção):** as rotas de intelligence passam a aceitar `project_id` além de `project_name`, com resolução dual-key org-escopada (`resolve_project_reference` + exceções tipadas) mapeada no boundary (id inexistente/cross-org → 404, id≠nome → 409, nome ambíguo → 409); nome único vira filtro exato por id, nome nunca-analisado preserva a lista vazia (nunca 404); migração `0014` re-backfilla `project_id` de forma defensiva (sem NOT NULL). Achado: `uq_projects_org_name` já torna a ambiguidade de nome estruturalmente impossível. **Etapas 2-5 pendentes**; o gate destrutivo da Etapa 4 (`DROP COLUMN`) exige nova aprovação explícita do Founder. |
| 9-13, — | Demais itens + Policies/Claims | Pendentes, na ordem da Seção 6. |
