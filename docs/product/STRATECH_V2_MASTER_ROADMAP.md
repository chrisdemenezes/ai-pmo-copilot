...
| Programa (= Domínio do Blueprint) | Objetivo | Dependências | Épicos/Releases relacionados | Status atual | Critério de conclusão |
|---|---|---|---|---|---|
| **Portfolio, Program & Project Intelligence** (inclui Projects, Portfolio, Actions, Decisions) | Projeto como entidade real; portfólio e programa como agregações; ações e decisões como objetos de primeira classe | Enterprise Foundation (Organização, RBAC) | Épico 4 (Release 0.1); expansão em Release 0.2 | **Em andamento** — Capability 01 implementou Portfolio e Capability 02 implementou Program (`web/lib/domain/{portfolio,program}.ts`), ambos como entidades reais de domínio no frontend, Program já vinculado a Portfolio (DDD, `portfolioId` obrigatório); nenhum dos dois tem persistência em banco ainda; `analysis_records.project_name` continua texto livre (migração para `Project` real é o Épico 4, ainda não iniciado); vínculo Program→Project é a Capability 03 | Portfolio/Program modelados, Projeto com ciclo de vida completo, ações/decisões vinculadas a entidades reais (não a texto livre) |
...
| Portfolio/Program como agregações | Em andamento (Portfolio + Program) | ~55% | Projeto como entidade real | Release 0.2, Capabilities 01 e 02. Portfolio e Program implementados como entidades reais de domínio no frontend (`web/lib/domain/{portfolio,program}.ts`), Program já vinculado a Portfolio (DDD) — adiantado explicitamente pelo Founder em relação à dependência formal (Projeto como entidade real, Épico 4, ainda não concluído). Sem persistência em banco, sem impacto no backend. Vínculo Program→Project é a Capability 03. |
...
**Resumo:** 3 de 17 capacidades completas (100%), 4 em andamento parcial (90% / 33% / ~55% / ~15%), 10 não iniciadas (0%). Média simples ≈ 29%. Recalculo completo do Indicador Global de Maturidade (Seção 8) e do Executive Dashboard (≈26%) segue postergado (ver nota da Capability 01) para não interromper a implementação apenas por reconciliação numérica — será feito quando a Capability 03 concluir o vínculo Program→Project.
