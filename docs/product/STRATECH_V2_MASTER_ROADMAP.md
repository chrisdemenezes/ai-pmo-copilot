...
| Programa (= Domínio do Blueprint) | Objetivo | Dependências | Épicos/Releases relacionados | Status atual | Critério de conclusão |
|---|---|---|---|---|---|
| **Portfolio, Program & Project Intelligence** (inclui Projects, Portfolio, Actions, Decisions) | Projeto como entidade real; portfólio e programa como agregações; ações e decisões como objetos de primeira classe | Enterprise Foundation (Organização, RBAC) | Épico 4 (Release 0.1); expansão em Release 0.2 | **Em andamento** — Capability 01 (Release 0.2) implementou Portfolio como entidade real de domínio no frontend (`web/lib/domain/portfolio.ts`), ainda sem persistência em banco; `analysis_records.project_name` continua texto livre (migração para `Project` real é o Épico 4, ainda não iniciado); Program não existe ainda | Portfolio/Program modelados, Projeto com ciclo de vida completo, ações/decisões vinculadas a entidades reais (não a texto livre) |
...
| Portfolio/Program como agregações | Em andamento (Portfolio) | ~25% | Projeto como entidade real | Release 0.2, Capability 01. Portfolio implementado como entidade real de domínio no frontend (`web/lib/domain/portfolio.ts`), adiantado explicitamente pelo Founder em relação à dependência formal (Projeto como entidade real, Épico 4, ainda não concluído) — sem persistência em banco, sem impacto no backend. Program ainda não iniciado. |
...
**Resumo:** 3 de 17 capacidades completas (100%), 4 em andamento parcial (90% / 33% / ~25% / ~15%), 10 não iniciadas (0%). Média simples ≈ 27%. Recalculo completo do Indicador Global de Maturidade (Seção 8) e do Executive Dashboard (≈26%) fica para a próxima atualização deste Roadmap, para não interromper a implementação da Capability 01 apenas por reconciliação numérica.
