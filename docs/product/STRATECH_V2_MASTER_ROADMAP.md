...
| **Última atualização factual** | RC-2 Enterprise Certification (`docs/product/governance/RC-2-ENTERPRISE-CERTIFICATION.md`) — Phase 1 (Enterprise Platform Foundation) encerrada; Phase 2 (Enterprise AI Platform) iniciada com uma proposta de Foundation Architecture (`docs/architecture/PHASE-2-FOUNDATION-ARCHITECTURE.md`, pendente de aprovação). **Nota de reconciliação:** a Phase 2 assume reorganizar o restante desta Release Roadmap (Releases 0.3-0.5 abaixo) — ainda não confirmado; as seções de Release abaixo permanecem como estavam até essa confirmação. |
