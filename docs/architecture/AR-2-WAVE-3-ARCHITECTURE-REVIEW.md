...
### Epic W3-2 — AI Platform Foundation (Provider Strategy extension + Model Registry + Prompt Versioning) — **Pronto para iniciar**

Sequenciamento já recomendado pelo próprio `DOMAIN-BLUEPRINT-WAVE-3-ENTERPRISE-INTELLIGENCE.md` §1: evoluir `LLMProvider`/`factory.py` de "1 provider por configuração" para múltiplos providers registrados (extensão do Protocol existente, não um segundo mecanismo); Model Registry (catálogo de modelos disponíveis); Prompt Versioning (extensão aditiva ao `PromptRegistry` existente, nunca um Registry novo — CLAUDE.md). Cost/Token Governance e AI Observability ficam sequenciados **depois** deste Epic (dependem da instrumentação básica existir primeiro), não incluídos neste Epic.
