...
## 3. Papel definitivo do `AIContextEngine` (redação institucional oficial — emenda D-086)

> **Definição oficial da plataforma (Founder Decision, D-086):** "AIContextEngine é o responsável pela preparação do contexto de IA. Sua responsabilidade inclui coletar, normalizar, consolidar e estruturar evidências para consumo dos Enterprise Advisors, sem executar regras de negócio nem interpretar domínio."

Esta definição substitui a redação original desta revisão ("coletor de evidências, nunca organizador de contexto") e passa a ser a definição institucional definitiva do `AIContextEngine` em toda a plataforma STRATECH. A fronteira que a revisão original protegia — nunca interpretar domínio, nunca executar regra de negócio — permanece intacta e explícita na própria redação oficial; o que muda é o reconhecimento formal de que preparar contexto abrange mais do que uma única chamada de coleta.

**Alinhamento com o código real, responsabilidade por responsabilidade:**
- **Coletar** — `AIContextEngine.gather()` (`src/services/ai_foundation/context_engine.py`) resolve `AnalysisRecord`s reais por `organization_id`/escopo/`kind` e os empacota em `Evidence` opacos (`summary: dict`, deliberadamente não interpretado pela Foundation, per o docstring do `Evidence`). Inalterado por esta emenda.
- **Normalizar** — cada `Evidence` retornado já é uma forma normalizada e opaca (mesmo formato de envelope, independentemente da origem ser `AnalysisRecord` ou, para a Classe D, um `Chunk` de RAG) — o "normalizar" da definição oficial é este envelope único, não uma transformação de domínio.
- **Consolidar** — quando um Advisor de escopo agregado (Classe B, §4) precisa de evidência de múltiplos projetos ou `kind`s, a consolidação (reunir as listas de `Evidence` resultantes de múltiplas chamadas a `gather_context()`) é uma responsabilidade da camada de preparação de contexto — hoje exercida pela etapa de montagem de contexto do próprio Advisor (§2), podendo evoluir para um método fino adicional do `AdvisorFramework`/`AIContextEngine` (ex.: `gather_context_many()`) caso um Advisor real da Classe B demonstre essa necessidade — sem que isso signifique conhecer o vocabulário de negócio de nenhum Advisor especificamente.
- **Estruturar** — o formato de saída (`list[Evidence]`) já é a estrutura entregue aos Advisors; estruturar não significa produzir `risks_json` ou qualquer narrativa pronta para prompt — essa continua sendo, sem exceção, uma responsabilidade do próprio Advisor (`RiskAdvisorAgent.advise()`), pois exige interpretar o que é um "risco" ou uma "composição de portfólio", vocabulário de domínio que a definição oficial explicitamente proíbe o `AIContextEngine` de conhecer.

**Por que isso não reabre o risco que a redação original evitava:** a definição oficial mantém, em sua própria frase final, a mesma garantia estrutural desta revisão — "sem executar regras de negócio nem interpretar domínio". Coletar, normalizar, consolidar e estruturar evidência são operações sobre a forma e a origem da evidência (quantas fontes, qual formato de envelope), nunca sobre o significado dela. A "organização" que a redação original vedava — transformar evidência bruta em narrativa de domínio pronta para prompt — permanece, sem ambiguidade, do lado do Advisor.

**Extensão necessária, não substituição:** para atender Advisors com escopo diferente de "um projeto, um kind" (Portfolio, PMO, Executive — ver §4), a resolução correta **não é generalizar `gather()` para conhecer domínio** — é consolidar múltiplas chamadas (hoje na etapa de montagem de contexto do Advisor, podendo migrar para um passthrough fino do Framework quando um caso real o exigir), sempre preservando o Engine como um componente que nunca precisa saber o que é "portfólio" ou "risco" para fazer seu trabalho.
...
**Atualização (D-086):** o Founder aprovou o AR-8 nestes termos (com a emenda do §3 acima) e decidiu que o primeiro Domain Blueprint da Wave 5 será o **Document Advisor** — o único Advisor da Classe D que já possui, hoje, produtor real de evento (`document.indexed`), Knowledge Platform concluída, RAG Pipeline concluído, Event Pipeline validado (Wave 4) e Workflow Runtime validado, representando a validação mais completa do modelo "Framework-Mediated Evidence Assembly" (§9) antes dos demais Advisors.
