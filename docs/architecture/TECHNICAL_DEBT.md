...
- **Status:** Deferred (confirmado por "Founder Decision — Technical Design do Document Advisor").
- **Descrição:** `AdvisorFramework.run()` lê `model_output.get("cited_analysis_ids")` literalmente (`framework.py:98`) para popular `RecommendationEngine.build()` — nome herdado do Risk Advisor, anterior ao rename de `Evidence.source_analysis_id` → `source_id` (D-088). Para o Document Advisor, os valores retornados por essa chave são `chunk_id`, não `analysis_id` — funcionalmente correto (ambos `int`, compatíveis com `source_id`), mas o nome da chave é enganoso para quem lê o prompt/código sem o contexto histórico. Corrigir exigiria alterar `AdvisorFramework.run()`, que o Founder determinou explicitamente **inalterado** neste Epic (implementação do W5-1).
- **Gatilho de resolução (oficializado pelo Founder):** o segundo Advisor baseado em RAG (Governance Advisor ou equivalente, Classe D per AR-8 §4) — nesse momento, com dois consumidores reais do mesmo padrão, avaliar renomear a chave para algo genérico (ex.: `"cited_source_ids"`) em uma mudança isolada a `AdvisorFramework.run()`, sem risco de "Generalized before Grounded". Nenhuma alteração a `AdvisorFramework.run()` antes desse gatilho.
