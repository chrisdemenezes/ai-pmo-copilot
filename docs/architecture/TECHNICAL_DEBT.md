...
- **Progresso:** per `DOMAIN-BLUEPRINT-PROJECT.md` (Opção A, faseada) — **Fase 1 concluída** (Sprint 1): os campos de domínio vivem na mesma tabela `projects` do Épico 1, não em uma `projects_delivery` separada. **Fase 2 concluída** (Sprint 5): o frontend lê da API real (arrays semeados deletados), e a migração `0008_domain_seed` executou a unificação in-place para os Projects legados com nome colidente ("Multilift"/"Aurora" — atualizados, nunca duplicados). **Fase 3 (Wave 3/AI Foundation)**: reconciliar `analysis_records.project_name` com `project_id`, aposentando `ProjectSummary` — o único dos 3 conceitos originais ainda desconectado.
